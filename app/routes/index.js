...
router.post('/reservasi/add', auth('mahasiswa'), async (req, res) => {
    try {
        ...
        if (!isDayInRange(tanggal, psi[0].jadwal_tugas)) {
            return res.send(`<script>alert('Gagal! ${psi[0].nama_psikolog} tidak bertugas hari itu.'); window.history.back();</script>`);
        }
...
// --- PSIKOLOG DASHBOARD (Dengan Pesan Tolak) ---
...
router.post('/psikolog/update-status', auth('psikolog'), async (req, res) => {
    ...
    try {
        if (status === 'ditolak') {
            await db.query('UPDATE reservasi SET status = ?, pesan_tolak = ? WHERE id = ?', ['ditolak', pesan_tolak, resId]);
        } else {
            await db.query('UPDATE reservasi SET status = ?, pesan_tolak = NULL WHERE id = ?', [status, resId]);
        }
        res.redirect(`/psikolog/${psiId}`);
    } catch (err) { res.status(500).send("Gagal"); }
});
